...
vi.mock('@/db/repositories/app-repository', () => ({
  appRepository: {
    ...
    upsertSyncQueueRecord: vi.fn(async (record: SyncQueueRecord) => {
      store.syncQueue = [...store.syncQueue.filter((entry) => entry.id !== record.id), record];
    }),
...
describe('cloud sync merge helpers', () => {
  beforeEach(() => {
    ...
    store.deletions = [];
    store.syncQueue = [];
    authState.currentUser = { uid: 'approved-user' };
  ...
  it('downloads cloud-only entities without clearing unrelated local data', async () => {
    ...
    expect(result.items.map((entry) => entry.id).sort()).toEqual(['cloud-only', 'local-only']);
  ...
  it('newer queued update supersedes older queued update for the same entity', async () => {
    ...
    await expect(
      queueCloudUpsert('item', withUpdatedAt(item('item-1'), '2026-07-13T09:00:00.000Z'))
    ).rejects.toThrow();
    await expect(
      queueCloudUpsert('item', withUpdatedAt(item('item-1'), '2026-07-13T10:00:00.000Z'))
    ).rejects.toThrow();
  ...
  it('delete supersedes a pending upsert', async () => {
    ...
    await expect(
      queueCloudUpsert('document', documentRecord('doc-1', '2026-07-13T09:00:00.000Z'))
    ).rejects.toThrow();
  ...
  it('queues signed-out changes without reading or writing Firestore', async () => {
    authState.currentUser = null;

    await expect(
      queueCloudUpsert('item', item('signed-out-item', '2026-07-13T10:00:00.000Z'))
    ).rejects.toThrow('Firebase Authentication is required');

    expect(store.syncQueue).toEqual([
      expect.objectContaining({ entityId: 'signed-out-item', attempts: 1 }),
    ]);
    expect(firestoreMocks.getDoc).not.toHaveBeenCalled();
    expect(firestoreMocks.setDoc).not.toHaveBeenCalled();
    expect(firestoreMocks.deleteDoc).not.toHaveBeenCalled();
  ...
  it('keeps only failed operations queued and a later retry removes only its success', async () => {
    ...
    await expect(
      queueCloudUpsert('item', item('item-failed', '2026-07-13T10:00:00.000Z'))
    ).rejects.toThrow();
    cloud.failingIds.add('item-success');
    await expect(
      queueCloudUpsert('item', item('item-success', '2026-07-13T10:00:00.000Z'))
    ).rejects.toThrow();
