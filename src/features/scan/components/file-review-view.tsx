...
export function FileReviewView({ documentId }: { documentId: string }) {
  const queuedFile = useAppStore((state) =>
    state.scanQueue.find((entry) => entry.documentId === documentId)
  );
        ...
        {!file ? (
          ...
          <>
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <p
                className={`mt-1 text-xs font-semibold ${file.confidence === 'high' ? 'text-emerald-700' : file.confidence === 'review' ? 'text-amber-700' : 'text-rose-700'}`}
              >
              ...
              {(file.rawRows ?? []).length === 0 ? (
                ...
                <ul className="space-y-2">
                  {file.rawRows?.map((row) => (
                    ...
                    >
                      {row.childName ?? 'Unknown child'} • {row.category ?? 'Unknown category'} •{' '}
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                ...
                <ul className="space-y-1">
                  {file.importPreviewIssues.map((issue) => (
                    <li
                      key={issue.id}
                      className="rounded-md bg-amber-50 px-2 py-1 text-sm text-amber-800"
                    >
