...
const countExtractedItems = (file: ScanSessionFileRecord) => {
  return Object.values(file.importPreviewCategoryCounts ?? {}).reduce(
...
export function ReviewQueueView() {
  ...
  const buildDraftRows = (documentId: string) => {
    ...
    return (file.rawRows ?? []).map(
      (row) =>
        reviewDrafts.find(
          (draft) => draft.documentId === documentId && draft.rowIndex === row.rowIndex
        ) ?? row
    );
  };
