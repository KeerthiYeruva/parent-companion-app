...
export function ScanSummaryCards({ files, lastScanAt }: { files: ScanSessionFileRecord[]; lastScanAt?: string }) {
  const cards = [
    { label: "New Files", value: countByStatus(files, "new") },
    { label: "Changed", value: countByStatus(files, "changed") },
    { label: "Duplicates", value: countByStatus(files, "duplicate") },
    { label: "Needs Review", value: countByStatus(files, "review") },
  ];

  return (
    <section className="space-y-3">
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <h2 className="text-xl font-semibold text-slate-900">Scan & Imports</h2>
        <p className="text-sm text-slate-600">{lastScanAt ? `Last scan: ${new Date(lastScanAt).toLocaleString()}` : "No scans yet."}</p>
      </div>
