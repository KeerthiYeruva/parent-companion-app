...
export const plannerChildShortName = (name: string) => name.split(/\s+/).filter(Boolean)[0] ?? name;
...
export function ChildSwitcher() {
  ...
  const setSelectedChildIds = useAppStore((state) => state.setSelectedChildIds);
  ...
  }, [children, selectedChildIds.length, setSelectedChildIds]);
