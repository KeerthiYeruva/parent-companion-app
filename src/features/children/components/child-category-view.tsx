...
export function ChildCategoryView({
  childId,
  title,
  categories,
}: {
      ...
      >
        <div className="child-category-view__header mb-3 flex items-center justify-between gap-3">
          ...
          <span className="child-category-view__progress text-sm font-medium text-slate-600">
            {completionProgress(items).label}
          </span>
        </div>
        ...
        <div className="child-month-view__panel rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="child-month-view__weeks space-y-3">
            {monthItemsByWeek(items).map((week) => (
              <div
                key={week.key}
                className="child-month-view__week rounded-lg border border-slate-100 bg-slate-50 p-3"
              >
                <div className="child-month-view__week-header mb-3 flex items-center justify-between gap-3">
                  <h4 className="child-month-view__week-title text-sm font-semibold text-slate-800">
                    Week of {week.label}
                  </h4>
                  <span className="child-month-view__week-progress rounded-full bg-white px-2 py-1 text-xs font-medium text-slate-600">
                    {week.progress.label}
                  </span>
                </div>
            ...
            {items.length === 0 ? (
              <ItemList
                items={[]}
                emptyText="No items for this month."
                showChild={false}
                showCategory={false}
              />
            ) : null}
  ...
  return (
    <div className="child-task-list space-y-3">
      <div className="child-task-section child-task-list__open">
        <h4 className="child-task-section__title mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
          To Do
        </h4>
        <ItemList
          items={split.open}
      ...
      {split.completed.length > 0 ? (
        <div className="child-task-section child-task-list__completed">
          <h4 className="child-task-section__title mb-2 text-xs font-semibold uppercase tracking-wide text-emerald-700">
            Completed
          </h4>
          <ItemList
