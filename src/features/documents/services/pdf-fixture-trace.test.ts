...
const extractFixtureRows = async (fileName: string, childName: string) => {
...
describe('real Unit Test portion PDF fixtures', () => {
  it('extracts all seven Grade 1 table subjects with complete portions', async () => {
    const rows = await extractFixtureRows('Grade_1_Unit_Test_Portion_20262027_9289.pdf', 'Grade 1');
    ...
    expect(rows.find((row) => row.subject === 'Science')?.title).toContain('How Things Move');
    expect(rows.find((row) => row.subject === 'Computer Science')?.title).toContain(
      'Using Computers'
    );
    expect(rows.find((row) => row.subject === 'General knowledge')?.title).toContain(
      'Me and My Family'
    );
    expect(rows.find((row) => row.subject === 'Kannada')?.title).toContain('Letters');
  });

  it('extracts all eight Grade 5 subjects across the real two-page PDF', async () => {
    const rows = await extractFixtureRows('GRADE__5__UNIT_TEST_1_PORTION__20262027.pdf', 'Grade 5');
    ...
    expect(rows.every((row) => row.title && row.description)).toBe(true);
    expect(rows.find((row) => row.subject === 'Social Studies')?.title).toContain('The DRC');
    expect(rows.find((row) => row.subject === 'Computer Science')?.title).toContain(
    ...
    expect(rows.find((row) => row.subject === 'Kannada')?.title).toContain('Creative writing');
  });
  ...
  it('automatically produces 15 ready child-specific items from all four PDFs', async () => {
    const [grade1Schedule, grade5Schedule, grade1Portions, grade5Portions] = await Promise.all([
      extractFixtureRows('GRADE_1JULY_SCHOLASTIC_PLANNER___20262724626_2481.pdf', 'Grade 1'),
      extractFixtureRows('GRADE_5_JULY_SCHOLASTIC_PLANNER_202627_1_4603.pdf', 'Grade 5'),
      extractFixtureRows('Grade_1_Unit_Test_Portion_20262027_9289.pdf', 'Grade 1'),
      extractFixtureRows('GRADE__5__UNIT_TEST_1_PORTION__20262027.pdf', 'Grade 5'),
    ]);
    ...
    expect(result.items.filter((item) => item.childId === 'grade-1-child')).toHaveLength(7);
    expect(result.items.filter((item) => item.childId === 'grade-5-child')).toHaveLength(8);
    expect(
      result.items.every(
        (item) =>
          item.dueDate &&
    ...
    expect(
      result.items.some(
        (item) => item.childId === 'grade-1-child' && item.subject === 'Social Studies'
      )
    ).toBe(false);
    expect(
      result.items.some(
        (item) => item.childId === 'grade-5-child' && item.subject === 'Social Studies'
      )
    ).toBe(true);
  });
});
