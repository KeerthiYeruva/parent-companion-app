...
export const formatSchoolDocumentTitle = (fileName: string, detectedType: DocumentType | "Unknown") => {
  ...
  return [gradeLabel, typeLabel === "School Document" && fallback ? fallback : typeLabel].filter(Boolean).join(" ");
};
