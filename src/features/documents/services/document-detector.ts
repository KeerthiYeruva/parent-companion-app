...
import type {
  FolderScanFile,
  PlannerExtractionSummary,
} from '@/features/documents/types/document-intelligence';

const extractChildHints = (relativePath: string, contentText?: string) => {
  const hints = new Set<string>();
  const source = `${relativePath} ${contentText ?? ''}`;
  const gradeRangeMatch =
    source.match(
      /\b(?:grades?|classes?)[\s_-]*[-:]?\s*(?:[0-9]{1,2}|i{1,3}|iv|v|vi{0,3}|ix|x|xi|xii)\s*(?:-|–|to)\s*(?:[0-9]{1,2}|i{1,3}|iv|v|vi{0,3}|ix|x|xi|xii)\b/gi
    ) ?? [];
  const gradeMatch =
    source.match(
      /\b(?:grade|class)\s*[-:]?\s*(?:[0-9]{1,2}|i{1,3}|iv|v|vi{0,3}|ix|x|xi|xii)\b/gi
    ) ?? [];
...
export const detectPlannerDocument = async (
  file: FolderScanFile
): Promise<PlannerExtractionSummary & { fileHash: string }> => {
